...
def parse_args():
	...
	parser.add_argument('--output_path', type=str,  default='inversion/inv', help='path to inference outputs')
	parser.add_argument('--gt_path', type=str, default='GT/real', help='path to gt images')
...
def run_on_step_output(step, args):
    ...
    for f in os.listdir(step_outputs_path):
        ...
        gt_path = os.path.join(args.gt_path, 'real' ,  f)

        if f.endswith(".jpg") or f.endswith('.png') or f.endswith('.jpeg'):
            file_paths.append([image_path, gt_path])
    print('len',len(file_paths))
