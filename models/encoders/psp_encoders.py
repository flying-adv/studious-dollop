...
class ResidualAligner(Module):
    ...
    def forward_inference(self , x):
        for i in reversed(range(self.opts.n_timestep)):
            loss , x_recon = self.diffusion.p_loss(self.model, x, torch.full((x.shape[0],), i, dtype=torch.int64).to(self.device))        
        return loss , x_recon
    
    def forward_combined(self , x):
        img = x
        imgs = p_sample_loop(self.diffusion , self.model , img , self.device , capture_every=1)
        return 0 , imgs[-1]
