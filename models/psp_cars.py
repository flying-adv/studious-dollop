...
class pSp(nn.Module):
    ...
    def load_weights(self):
        if self.opts.checkpoint_path is not None:
            ...
            if not self.opts.is_train:
                ...
                self.grid_align.ema.load_state_dict(get_keys(ckpt, 'ema'), strict=True)
    ...
    def forward(self, x, resize=True, latent_mask=None, input_code=False, randomize_noise=True,
        ...
        imgs_ = torch.nn.functional.interpolate(torch.clamp(images, -1., 1.), size=(192,256) , mode='bilinear') 
        res_gt = (x - imgs_ ).detach() 
        # res_unaligned = self.grid_transform(res_gt).detach() 
    

        loss_diff , res_aligned = self.grid_align(torch.cat((res_gt, imgs_ ), 1))
        res_aligned = res_aligned.to(self.opts.device)
        res = res_aligned + torch.cat((res_gt, imgs_  ), 1)      
        ...
        if return_latents:
            return images, result_latent, 0, imgs_ , loss_diff
        else:
            return images , loss_diff
