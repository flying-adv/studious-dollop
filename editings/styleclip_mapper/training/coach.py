    ...
    def train(self):
        ...
        while self.global_step < self.opts.max_steps:
            for batch_idx, batch in enumerate(self.train_dataloader):
                ...
                # with torch.no_grad():
                x_hat , _ = self.net.net.decoder([w_hat] , None, input_is_latent=True, randomize_noise=False, return_latents=True)
                img_edit = torch.nn.functional.interpolate(torch.clamp(x_hat, -1., 1.), size=(256,256) , mode='bilinear')
                res_ = x - img_edit
                
                _ , res_align  = self.net.net.grid_align(torch.cat((res_, img_edit), 1))
                res_align = res_align + torch.cat((res_, img_edit  ), 1)      

                conditions = self.net.net.residue(res_align)           
    ...
    def validate(self):
        ...
        for batch_idx, batch in enumerate(self.test_dataloader):
            ...
            with torch.no_grad():
                ...
                _ , res_align_  = self.net.net.grid_align(torch.cat((res_, img_edit  ), 1))
                res_align_ = res_align_ + torch.cat((res_, img_edit  ), 1)  
