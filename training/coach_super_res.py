...
# from models.psp_cars import pSp
...
class CascadedSuperResolutionNetwork(nn.Module):
    def __init__(self, stylegan_channels):
        super(CascadedSuperResolutionNetwork, self).__init__()
        hidden_dim = 256
        self.stage_blocks = nn.ModuleList([
            SuperResolutionBlock(stylegan_channels, hidden_dim),
            SuperResolutionBlock(hidden_dim, hidden_dim),
            UpSamplingBlock(hidden_dim, hidden_dim),
            SuperResolutionBlock(stylegan_channels, hidden_dim),
            SuperResolutionBlock(hidden_dim, hidden_dim),
            UpSamplingBlock(hidden_dim, hidden_dim),
            SuperResolutionBlock(stylegan_channels, hidden_dim),
            ...
            UpSamplingBlock(hidden_dim, hidden_dim),
            SuperResolutionBlock(stylegan_channels, hidden_dim),
            SuperResolutionBlock(hidden_dim, 3),                                    
        ])
        
        # Apply initialization to all layers in the network
...
class Coach:
    def __init__(self, opts):
        ...
        self.super_res = CascadedSuperResolutionNetwork(9).to(self.device)
        #### for faces ####
    ...
    def train(self):
        ...
        while self.global_step < self.opts.max_steps:
            for batch_idx, batch in enumerate(self.train_dataloader):
                loss_dict = {}
                with torch.no_grad():
                    x, y, y_hat, latent, res_delta, rec , loss_diff,res = self.forward(batch)
                y_hat = self.super_res(y_hat,res)
    ...
    def validate(self):
        ...
        for batch_idx, batch in enumerate(self.test_dataloader):
            ...
            with torch.no_grad():
                ...
                cur_loss_dict['diffusion'] = loss_diff.item()
    ...
    def configure_optimizers(self):
        params = list(self.super_res.parameters())
    ...
    def calc_loss(self, x, y, y_hat, latent, res_delta):
        ...
        if self.opts.l2_lambda > 0:
            loss_l2 = F.l1_loss(y_hat, y)
            loss_dict['loss_l1'] = float(loss_l2)
    ...
    def forward(self, batch):
        ...
        y_hat, latent, res_delta, rec , loss_diff, res = self.net.forward(x, return_latents=True)
        if self.opts.dataset_type == "cars_encode":
            y_hat = y_hat[:, :, 32:224, :]
        return x, y, y_hat, latent, res_delta, rec , loss_diff, res
    ...
    def __get_save_dict(self):
        save_dict = {
            'state_dict': self.super_res.state_dict(),
            'opts': vars(self.opts)}
